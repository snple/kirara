...
func (s *AttrService) Create(ctx context.Context, in *pb.Attr) (*pb.Attr, error) {
	...
	// class validation
	{
		class, err := s.cs.GetClass().ViewByID(ctx, in.GetClassId())
		if err != nil {
			return &output, err
		}
	...
	// tag validation
	if in.GetTagId() != "" {
		tag, err := s.cs.GetTag().ViewByID(ctx, in.GetTagId())
		if err != nil {
			return &output, err
		}
...
func (s *AttrService) List(ctx context.Context, in *cores.AttrListRequest) (*cores.AttrListResponse, error) {
	var err error
	var output cores.AttrListResponse
...
func (s *AttrService) Clone(ctx context.Context, in *cores.AttrCloneRequest) (*pb.MyBool, error) {
	...
	err = s.cs.getClone().attr(ctx, s.cs.GetDB(), in.GetId(), in.GetClassId())
	if err != nil {
		return &output, err
	}
...
func (s *AttrService) Pull(ctx context.Context, in *cores.AttrPullRequest) (*cores.AttrPullResponse, error) {
	var err error
	var output cores.AttrPullResponse
...
func (s *AttrService) setValue(ctx context.Context, in *pb.AttrValue, check bool) (*pb.MyBool, error) {
	...
	// validation device and class
	{
		...
		// class
		{
			class, err := s.cs.GetClass().ViewFromCacheByID(ctx, item.ClassID)
			if err != nil {
				return &output, err
			}
	...
	if item.TagID != "" {
		...
		return s.cs.GetTag().SetValueForce(ctx, &pb.TagValue{Id: item.TagID, Value: in.GetValue()})
	}
...
func (s *AttrService) setValueByName(ctx context.Context, in *cores.AttrNameValue, check bool) (*pb.MyBool, error) {
	...
	// class
	class, err := s.cs.GetClass().ViewFromCacheByDeviceIDAndName(ctx, device.ID, className)
	if err != nil {
		return &output, err
	}
...
func (s *AttrService) Upload(ctx context.Context, in *cores.AttrValueUploadRequest) (*cores.AttrValueUploadResponse, error) {
	var output cores.AttrValueUploadResponse
	var err error
	...
	if err != nil {
		return &output, status.Errorf(codes.InvalidArgument, "nson::Array::Decode: %v", err)
	}
	...
	if arrayLen%2 != 0 {
		return &output, status.Error(codes.InvalidArgument, "Please supply valid content format")
	}

	for i := 0; i < len(array); i += 2 {
		if array[i].Tag() != nson.TAG_MESSAGE_ID {
			return &output, status.Error(codes.InvalidArgument, "ID type != nson.TAG_MESSAGE_ID")
		}

		id := array[i].(nson.MessageId).Hex()
		value := array[i+1]

		if in.GetDeviceId() != "" {
			attr, err := s.ViewFromCacheByID(ctx, id)
			if err != nil {
				return &output, err
			}

			if attr.DeviceID != in.GetDeviceId() {
				return &output, status.Errorf(codes.InvalidArgument,
					"attr.DeviceID != DeviceID, Attr.ID: %v, DeviceID: %v", id, in.GetDeviceId())
			}
		}

		s.setAttrValue(id, value)
	}
