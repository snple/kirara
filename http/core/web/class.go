...
func (s *ClassService) list(ctx *gin.Context) {
	...
	reply, err := s.ws.Core().GetClass().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *ClassService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Core().GetClass().View(ctx, request)
	if err != nil {
...
func (s *ClassService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Core().GetClass().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *ClassService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Core().GetClass().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *ClassService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Core().GetClass().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
