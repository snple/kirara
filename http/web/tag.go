...
func (s *TagService) list(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetTag().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *TagService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetTag().View(ctx, request)
	if err != nil {
...
func (s *TagService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetTag().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *TagService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Edge().GetTag().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *TagService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Edge().GetTag().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
