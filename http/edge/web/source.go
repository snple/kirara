...
func (s *SourceService) list(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetSource().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *SourceService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetSource().View(ctx, request)
	if err != nil {
...
func (s *SourceService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetSource().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *SourceService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Edge().GetSource().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *SourceService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Edge().GetSource().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
