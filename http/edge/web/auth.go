...
import (
	...
	"github.com/snple/kirara/http/util/jwt"
...
func newAuthService(ws *WebService) (*AuthService, error) {
	s := &AuthService{
		...
		cache: cache.NewCache(func(ctx context.Context, key string) (*pb.User, time.Duration, error) {
			n, err := ws.Edge().GetUser().View(ctx, &pb.Id{Id: key})
			if err != nil {
				return nil, 0, err
			}
...
func (s *AuthService) Authenticator(ctx *gin.Context) (interface{}, error) {
	...
	request := &edges.LoginRequest{
		Name: loginVals.User,
		Pass: loginVals.Pass,
	}

	reply, err := s.ws.Edge().GetAuth().Login(ctx, request)
	if err != nil {
		return "", err
	}
...
func (s *AuthService) changePass(ctx *gin.Context) {
	...
	var params edges.ChangePassRequest
	...
	reply, err := s.ws.Edge().GetAuth().ChangePass(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *AuthService) forceChangePass(ctx *gin.Context) {
	...
	var params edges.ForceChangePassRequest
	...
	reply, err := s.ws.Edge().GetAuth().ForceChangePass(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
