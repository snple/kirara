...
func (s *LogicService) list(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetLogic().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *LogicService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetLogic().View(ctx, request)
	if err != nil {
...
func (s *LogicService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetLogic().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *LogicService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Edge().GetLogic().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *LogicService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Edge().GetLogic().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
