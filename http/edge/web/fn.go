...
func (s *FnService) list(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetFn().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *FnService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetFn().View(ctx, request)
	if err != nil {
...
func (s *FnService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetFn().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *FnService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Edge().GetFn().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *FnService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Edge().GetFn().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
