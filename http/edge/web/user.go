...
func (s *UserService) list(ctx *gin.Context) {
	...
	request := &edges.UserListRequest{
		Page: page,
		Tags: params.Tags,
		Type: params.Type,
	}

	reply, err := s.ws.Edge().GetUser().List(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *UserService) get(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetUser().View(ctx, request)
	if err != nil {
...
func (s *UserService) post(ctx *gin.Context) {
	...
	reply, err := s.ws.Edge().GetUser().Create(ctx, &params)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *UserService) patch(ctx *gin.Context) {
	...
	reply2, err := s.ws.Edge().GetUser().Update(ctx, reply)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
...
func (s *UserService) delete(ctx *gin.Context) {
	...
	_, err := s.ws.Edge().GetUser().Delete(ctx, request)
	if err != nil {
		ctx.JSON(util.Error(400, err.Error()))
		return
	}
