...
type ClassService struct {
	...
	cache *cache.Cache[model.Class]

	edges.UnimplementedClassServiceServer
}

func newClassService(es *EdgeService) *ClassService {
	return &ClassService{
		es:    es,
		cache: cache.NewCache[model.Class](nil),
	}
}
...
func (s *ClassService) Create(ctx context.Context, in *pb.Class) (*pb.Class, error) {
	...
	// name validation
	{
		...
		err = s.es.GetDB().NewSelect().Model(&model.Class{}).Where("name = ?", in.GetName()).Scan(ctx)
		if err != nil {
...
func (s *ClassService) Update(ctx context.Context, in *pb.Class) (*pb.Class, error) {
	...
	// basic validation
	{
		...
		if len(in.GetId()) == 0 {
			return &output, status.Error(codes.InvalidArgument, "Please supply valid ClassID")
		}
...
func (s *ClassService) List(ctx context.Context, in *edges.ClassListRequest) (*edges.ClassListResponse, error) {
	var err error
	var output edges.ClassListResponse
...
func (s *ClassService) Clone(ctx context.Context, in *edges.ClassCloneRequest) (*pb.MyBool, error) {
	...
	err = s.es.getClone().class(ctx, tx, in.GetId())
	if err != nil {
		return &output, err
	}
...
func (s *ClassService) ViewByName(ctx context.Context, name string) (model.Class, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return item, status.Errorf(codes.NotFound, "Query: %v, class Name: %v", err, name)
		}
...
func (s *ClassService) Pull(ctx context.Context, in *edges.ClassPullRequest) (*edges.ClassPullResponse, error) {
	var err error
	var output edges.ClassPullResponse
