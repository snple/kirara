...
func (s *DataService) Upload(ctx context.Context, in *edges.DataUploadRequest) (*edges.DataUploadResponse, error) {
	...
	device, err := s.es.GetDevice().ViewByID(ctx)
	if err != nil {
		return &output, err
	}
...
func (s *DataService) updateValue(tag *model.Tag, value nson.Value) {
	...
	updateValue := func(tag *model.Tag, value nson.Value) {
		...
		if err = s.es.GetTag().setTagValueUpdated(ctx, tag, valueString, time.Now()); err != nil {
			return
		}

		if err = s.es.GetTag().afterUpdateValue(ctx, tag, valueString); err != nil {
			return
		}
	}
...
func (s *DataService) uploadContentType1(ctx context.Context, in *edges.DataUploadRequest, _ *edges.DataUploadResponse) error {
	...
	for ; i < len(array); i += 2 {
		...
		tag, err := s.es.GetTag().ViewByID(ctx, tagID)
		if err != nil {
			return err
		}
		...
		source, err := s.es.GetSource().ViewFromCacheByID(ctx, tag.SourceID)
		if err != nil {
			return err
		}
...
func (s *DataService) uploadContentType2(ctx context.Context, in *edges.DataUploadRequest, _ *edges.DataUploadResponse) error {
	// {
	//  [TIMESTAMP: u32,]
	//  [SOURCE: String,]
	...
	if ts, err := message.GetU32(UPLOAD_TIMESTAMP); err == nil {
		if ts > 0 {
			timestamp = int64(ts)
		}
	}
	...
	if st, err := message.GetString(UPLOAD_SOURCE); err == nil {
		sourceName = st
	}
	...
	if ty, err := message.GetU32(UPLOAD_TYPE); err == nil {
		if ty == UPLOAD_TYPE_NAME || ty == UPLOAD_TYPE_ADDRESS {
			uploadType = ty
		}
	}
	...
	for name, value := range message {
		if strings.HasPrefix(name, UPLOAD_PREFIX) {
			continue
		}
		...
		if uploadType == UPLOAD_TYPE_ADDRESS {
			tag, err = s.es.GetTag().ViewBySourceIDAndAddress(ctx, source.ID, name)
		} else {
			tag, err = s.es.GetTag().ViewBySourceIDAndName(ctx, source.ID, name)
		}
		if err != nil {
			return err
		}
...
func (s *DataService) uploadContentType3(ctx context.Context, in *edges.DataUploadRequest, _ *edges.DataUploadResponse) error {
	// {
	//  [SOURCE: String,]
	//  Type: u32, 0: Name, 1: Address, default 0
	//  Name: Name|Address,
	...
	// name
	name, err := message.GetString(UPLOAD_NAME)
	if err != nil {
		return status.Errorf(codes.InvalidArgument, "Get 'Name' Field: %v", err)
	}

	// values
	values, err := message.GetArray(UPLOAD_VALUES)
	if err != nil {
...
func (s *DataService) uploadContentType12(ctx context.Context, in *edges.DataUploadRequest, _ *edges.DataUploadResponse) error {
	// {
	//  [TIMESTAMP: u32,]
	//  [SOURCE: String,]
	//  Type: u32, 0: Name, 1: Address, default 0
	...
	if v, ok := message[UPLOAD_TIMESTAMP]; ok {
	...
	if v, ok := message[UPLOAD_SOURCE]; ok {
	...
	if v, ok := message[UPLOAD_TYPE]; ok {
		if n, ok := v.(json.Number); ok {
			tys, e := n.Int64()
			if e != nil {
