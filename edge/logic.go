...
type LogicService struct {
	...
	edges.UnimplementedLogicServiceServer
}

func newLogicService(es *EdgeService) *LogicService {
	return &LogicService{
		es: es,
	}
}
...
func (s *LogicService) Create(ctx context.Context, in *pb.Logic) (*pb.Logic, error) {
	...
	// name validation
	{
		...
		err = s.es.GetDB().NewSelect().Model(&model.Logic{}).Where("name = ?", in.GetName()).Scan(ctx)
		if err != nil {
...
func (s *LogicService) List(ctx context.Context, in *edges.LogicListRequest) (*edges.LogicListResponse, error) {
	var err error
	var output edges.LogicListResponse
...
func (s *LogicService) Clone(ctx context.Context, in *edges.LogicCloneRequest) (*pb.MyBool, error) {
	...
	err = s.es.getClone().logic(ctx, s.es.GetDB(), in.GetId())
	if err != nil {
		return &output, err
	}
...
func (s *LogicService) ViewByName(ctx context.Context, name string) (model.Logic, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return item, status.Errorf(codes.NotFound, "Query: %v, Logic.Name: %v", err, name)
		}
...
func (s *LogicService) afterUpdate(ctx context.Context, item *model.Logic) error {
	...
	err = s.es.GetSync().setLogicUpdated(ctx, time.Now())
	if err != nil {
		return status.Errorf(codes.Internal, "Insert: %v", err)
	}
...
func (s *LogicService) Pull(ctx context.Context, in *edges.LogicPullRequest) (*edges.LogicPullResponse, error) {
	var err error
	var output edges.LogicPullResponse
